...
// Function to get enabled clients from environment variables
function getEnabledClients(): Clients[] {
    const enabledClients: Clients[] = [];

    // Check each client's environment variable
    if (process.env.TELEGRAM_ENABLED === 'true') {
      enabledClients.push(Clients.TELEGRAM);
    }
    if (process.env.DISCORD_ENABLED === 'true') {
      enabledClients.push(Clients.DISCORD);
    }
    if (process.env.TWITTER_ENABLED === 'true') {
      enabledClients.push(Clients.TWITTER);
    }
    ...
    if (enabledClients.length === 0) {
      return [Clients.TELEGRAM]; // Using Telegram as default
    }
